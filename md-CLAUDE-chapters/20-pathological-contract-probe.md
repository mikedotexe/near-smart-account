...
`PromiseError::Failed` at `on_stage_call_settled`.
...
- **Layer 1 — NEAR receipt classification.** `trace-rpc.mjs`'s `classify()`
  ...
  marks as `Failure`.
- **Layer 2 — smart-account log interpretation.** `on_stage_call_settled`
...
`echo(0)` as decoy. Smart-account's `.then(on_stage_call_settled)`
chains on the decoy.
...
ceiling in `on_stage_call_settled`.
...
`scripts/probe-pathological.mjs`, which stages + releases in one
process — the stage tx uses `waitUntil: "INCLUDED"` so the script
...
`on_stage_call_resume` and `on_stage_call_settled`; inner+overhead must
...
The tx chosen for investigation is the STAGE tx — it's the one whose
receipt tree hosts the yielded resume callback + downstream call +
settle callback cascade. The run_sequence tx only shows the resume
...
- stage tx: `5EnfXYMJMzSBxsRn53osGHdFaBzoUJi8W54ZYcdVuGQn`
...
- gas_burnt (stage tx): 317 TGas
- cascade window: blocks 246336531..246336544 (14 blocks)
- state before → after: `calls_completed: 0 → 1`, `last_burst: null → "baseline"`
- settle log: `settled successfully via direct pathological-router.x.mike.testnet.do_honest_work (20 result bytes); sequence completed`
...
- stage tx: `FEa5Cuie2F6knyLEQ8AEJHySNUfd7zdcX2veaPrzkn6Y`
...
- settle log: `failed downstream via direct pathological-router.x.mike.testnet.burn_gas; ordered release stopped here: Failed`

Observation: gas exhaustion is caught at Layer 1 (the runtime marks the
burn_gas receipt as Failure) AND at Layer 2 (settle sees
...
variant at settle. The gas usage is observable (~315 TGas for this
...
- stage tx: `9GFisTDnuMvMsN28NHqxJvUAC3eVw9QB7G35FPaghQej`
...
- state before → after: unchanged (`calls_completed: 1 → 1`, `last_burst` unchanged)
- settle log: `settled successfully via direct pathological-router.x.mike.testnet.noop_claim_success (4 result bytes); sequence completed`

Observation: **Layers 1 and 2 both report SUCCESS**. The NEAR receipt
DAG shows every receipt `SuccessValue`. The smart-account's settle sees
...
- settle log: `settled successfully via direct pathological-router.x.mike.testnet.return_decoy_promise (1 result bytes); sequence completed`
...
SUCCESS (settle chained on the decoy, decoy returned cleanly). Layer 3
...
settle chain on, and did it succeed?" is fooled in both cases, but the
...
and the pathology becomes a real attack vector: settle advances on the
decoy while the detached work does its damage.

### §4.4 Pathology 4 — `return_oversized_payload(20)`

- stage tx: `73xuZ8Bo7VtchwhkPpAS3Ep4ZcfFUTsWTN3gak6xwcri`
...
- settle log: `failed downstream via direct pathological-router.x.mike.testnet.return_oversized_payload; ordered release stopped here: TooLong(20482)`
...
differently — but `on_stage_call_settled` in the current kernel
...
| Pathology | L1 (NEAR receipt class) | L2 (smart-account settle) | L3 (target state) | First detection at | Work actually done? |
|---|---|---|---|---|---|
...
- **oversize** is the only pathology where `Direct`'s Layer 2 gives
  ...
  actual observed size. Today's `on_stage_call_settled` discards that
  detail; a future kernel variant could surface it via a
  `SequenceHaltedOversize { step, size }` event.

## §6 Open questions (feeds v1.1)

- **Callback panics in settle.** If `on_stage_call_settled` itself
  panics (smart-account bug, not target bug), the yield stays in
  `staged_calls`, the sequence queue isn't cleaned up, and the next
