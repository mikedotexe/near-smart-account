...
Context pack for Codex sessions working in this repo. Read first; details
...
| `scripts/` | `check.sh`, `test.sh` alias, `build-all.sh`, `deploy-testnet.sh`, `send-staged-echo-demo.mjs`, `send-staged-mixed-demo.mjs`, `send-balance-trigger-router-demo.mjs` (repo-local direct / adapter / mixed modes), `send-balance-trigger-wrap-demo.mjs` (real `wrap.testnet` path), plus the internal FastNear observability toolkit (`trace-tx.mjs`, `investigate-tx.mjs`, `receipt-to-tx.mjs`, `account-history.mjs`, `watch-tip.mjs`, `block-window.mjs`, `state.mjs`). |
...
1. Sketch the design in the session (prose + sometimes AGENTS.md update).
...
- `md-CLAUDE-chapters/05-staged-call-three-surfaces.md` — the same
  observability method from chapter 04, applied to the smart-account staged
  run: contract state, per-block receipts, and why state drains on settle.
...
- `md-CLAUDE-chapters/11-orbital-model-diagrams.md` — the mental-model
  chapter: six diagrams of the sphere-and-satellites picture
  (hub/spoke, lifecycle, single cascade, halt-retry saga,
  cross-caller, four-fates flowchart) plus a glossary mapping the
  orbital vocabulary back to contract code.
- `md-CLAUDE-chapters/12-deterministic-smart-account-automation.md` — the
  ...
  deterministic receipt-control plane.
- `md-CLAUDE-chapters/13-stage-call-against-real-defi.md` — first live
  probe against a real DeFi contract (`wrap.testnet`): three-action
  staged batch runs end to end, answers the open questions about gas,
  byte-count, and cross-shard latency.
...
- `md-CLAUDE-chapters/15-stage-call-wild-contract-semantics.md` — the
  ...
  (`ft_transfer_call`) extends the cascade from 3 to 5 blocks per step;
  four distinct failure shapes all collapse to `PromiseError::Failed`
  at settle. Motivates chapter 14's `Adapter` policy from the
  empirical side.
...
- `md-CLAUDE-chapters/17-stage-call-multi-contract-intent.md` — first
  ...
  ordering enforced by one `run_sequence`. Teachable failure-then-success
  pair (Run A halts on `E21: token not registered` because Run A's
  deposit was bounced and refunded; Run B tops up storage and
  completes end-to-end, smart-account ends with 0.033 RFT in Ref's
  internal ledger).
- `md-CLAUDE-chapters/18-keep-yield-canonical.md` — the design note on why
  ...
  though a no-yield runner is mechanically possible.
